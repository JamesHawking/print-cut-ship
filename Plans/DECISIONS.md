...
| Auth | Hand-rolled in Go: argon2id + opaque tokens (hashed) + Postgres sessions, httpOnly cookie | No better-auth equivalent in Go; email+password+sessions on vetted primitives is a small, testable surface behind the OpenAPI contract | Go OAuth frameworks (authboss/goth — breadth we deferred); JWTs (revocation pain); ~~better-auth~~ (pre-pivot TS choice) |
| Payments | Stripe — Checkout/Elements with P24 + BLIK payment methods | **User-locked.** Best DX, hosted surfaces keep the app out of PCI scope, P24+BLIK cover Polish habits | Przelewy24/PayU direct — lower fees, materially worse DX |
