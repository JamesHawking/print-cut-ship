...
_Amended 2026-07-30:_ the object store is **Garage v2** (Coolify service, host 9002 → container 3900), not MinIO — community MinIO became unmaintained, and Garage was chosen over R2-primary to keep customer files on own infra. Garage **validates the SigV4 region**, so `storage.go` gained an optional `S3_REGION` env (set to `garage` in prod; empty keeps the client default for local compose MinIO). Buckets `instantquote` + `instantquote-dev`, key `iq-api`, CORS for presigned browser PUTs — all provisioned via the Garage CLI; there is no web console.
