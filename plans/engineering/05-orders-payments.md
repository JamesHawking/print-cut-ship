...
> **Status: ✅ Done** (2026-07-19, reviewed and merged to main in `abd9889` — as amended 2026-07-19).
> **Review (2026-07-19):** merged after two fixes — (1) `ApplyPaymentSucceeded`/`ApplyRefundSucceeded` now commit the payments ledger row even when the SQL guard refuses the status transition (typed `ErrTransitionRefused`; the pipeline logs loudly and acks), closing a double-charge-with-no-trace hole for plan 18's real events; (2) `PUBLIC_BASE_URL` default corrected to `:3000`. Non-blocking notes recorded: `orderStatus.paidTitle` i18n key unused; NIP-strips-to-empty B2B edge (backend 400s wrong-length NIPs); abandoned draft orders accumulate (plan 07/16 sweep concern).
